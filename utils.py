...
class AudDataset(object):
    def __init__(self, wavpath):
        ...
        self.data_len = int((self.orig_mel.shape[0] - 16) / 80.0 * float(25)) + 2
...
class AudDataset2(object):
    def __init__(self, wav):
        wav_16k = librosa.resample(wav, orig_sr=48000, target_sr=16000)
        self.orig_mel = melspectrogram(wav_16k).T
        self.data_len = int((self.orig_mel.shape[0] - 16) / 80.0 * 25) + 2

    def crop_audio_window(self, spec, start_frame):
        start_idx = int(80.0 * (start_frame / float(25)))
        end_idx = start_idx + 16
    ...
    def __getitem__(self, idx):
        mel = self.crop_audio_window(self.orig_mel.copy(), idx)
        if mel.shape[0] < 16:
            pad_size = 16 - mel.shape[0]
            mel = np.pad(mel, ((0, pad_size), (0, 0)), mode="constant")
